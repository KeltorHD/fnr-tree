...
public:
	...
	using mbr_t   = std::array<point_t, 2>;           /*лева€ нижн. точка и права€ верхн. точка mbr*/

	R_tree();
	~R_tree() = default;

	R_tree(const R_tree&) = delete;  /*конструкторы и операторы копировани€ и переноса удалены*/
	R_tree(R_tree&&) = delete;
	R_tree operator=(const R_tree&) = delete;
	R_tree operator=(R_tree&&) = delete;

	/*основные функции*/
	void insert(const data_type& data, const mbr_t& mbr); /*вставка нового значени€ в дерево с заданным mbr*/

private:
	struct node;
	using node_ptr_t        = std::shared_ptr<node>;                         /*тип указател€ на node*/
	using child_array_ptr_t = std::array<node_ptr_t, max_nodes>;             /*массив указателей на потомков*/
	using data_info_t       = std::pair<data_type, mbr_t>;                   /*полна€ информаци€ об объекте*/
	...
	struct node
	{
		mbr_t mbr{};                             /*минимальный ограничивающий пр€моугольник n-мерного пространства*/
	...
	/*функции дл€ работы с деревом*/
	node_ptr_t choice_leaf(const mbr_t& mbr) const;                                       /*поиск листа, в который можно поместить новое значение*/
	node_ptr_t node_division(node_ptr_t l1, const data_type& data, const mbr_t& mbr);     /*деление узла на 2 по квадратичному алгоритму √уттмана*/
	...
	coord_type diff_square_mbr(const mbr_t& m1, const mbr_t& m2) const; /*разница в площад€х двух mbr*/
};
...
inline typename R_class_area::mbr_t R_class_area::sum_mbr(const mbr_t& m1, const mbr_t& m2) const
{
	...
	{
		ld[i] = std::min(m1[0][i], m2[0][i]); /*лева€ нижн€€ точка*/
		ru[i] = std::max(m1[1][i], m2[1][i]); /*права€ верхн€€ точка*/
	}
	return mbr_t{ ld,ru };
}
...
inline void R_class_area::insert(const data_type& data, const mbr_t& mbr)
{
	...
	if (l->count_array < max_nodes) /*если значение помещаетс€ в текущий узел*/
	{
